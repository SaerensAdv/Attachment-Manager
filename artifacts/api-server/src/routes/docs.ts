...
import { GetDocGraphResponse, GetDocContentQueryParams, GetDocContentResponse, GetDocValidationResponse, GetDocBacklinksQueryParams, GetDocBacklinksResponse, SearchDocsBody, SearchDocsResponse } from "@workspace/api-zod";
import { getDocGraph, listDocFiles } from "../lib/docs";
import { getCompatibleDocFile, resolveCompatibleSourcePath } from "../lib/source-docs";
...
router.get("/docs/hierarchy", async (_req, res): Promise<void> => { const clientDocs = await loadClientDocs(); const result = loadBrainHierarchy(listDocFiles(clientDocs).map((file) => file.path)); res.status(result.issues.length ? 503 : 200).json(result); });
