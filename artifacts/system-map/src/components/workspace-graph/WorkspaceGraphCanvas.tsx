...
function useSize(ref: React.RefObject<HTMLDivElement | null>) {
  ...
  useEffect(() => { const element = ref.current; if (!element) return; const observer = new ResizeObserver(([entry]) => setSize({ width: entry.contentRect.width, height: entry.contentRect.height })); observer.observe(element); return () => observer.disconnect(); }, [ref]);
  return size;
}
...
export default function WorkspaceGraphCanvas({ nodes, edges, hiddenGroups, selectedNodeId, onSelectNode, fitKey, focusRequest }: WorkspaceGraphCanvasProps) {
  ...
  useEffect(() => {
    ...
    const nextNodes: SimNode[] = nodes.map((node) => ({ ...node, ...(positions.current.get(node.id) ?? {}) }));
    ...
    nextEdges.forEach((edge) => { degree.set(edge.source as string, (degree.get(edge.source as string) ?? 0) + 1); degree.set(edge.target as string, (degree.get(edge.target as string) ?? 0) + 1); });
    const simulation = d3.forceSimulation(nextNodes)
      .force("link", d3.forceLink<SimNode, SimEdge>(nextEdges).id((node) => node.id).distance((edge) => 78 + Math.min(12, Math.max(degree.get((edge.source as SimNode).id) ?? 0, degree.get((edge.target as SimNode).id) ?? 0)) * 6).strength(.72))
      .force("charge", d3.forceManyBody<SimNode>().strength((node) => -280 - (degree.get(node.id) ?? 0) * 45))
      .force("center", d3.forceCenter(size.width / 2, size.height / 2))
      .force("collide", d3.forceCollide<SimNode>().radius((node) => radius(node) + 28).iterations(2))
      .stop();
  ...
  const fit = useCallback((duration = 380) => {
    ...
    const xs = simNodes.map((node) => node.x ?? 0), ys = simNodes.map((node) => node.y ?? 0);
    const minX = Math.min(...xs), maxX = Math.max(...xs), minY = Math.min(...ys), maxY = Math.max(...ys);
    const nextScale = Math.min(.94, size.width / (maxX - minX + 260), size.height / (maxY - minY + 220));
    api.current.setTransform(size.width / 2 - ((minX + maxX) / 2) * nextScale, size.height / 2 - ((minY + maxY) / 2) * nextScale, nextScale, duration);
    setScale(nextScale);
  }, [simNodes, size]);
  ...
  const connected = useMemo(() => {
    ...
    simEdges.forEach((edge) => { const source = edge.source as SimNode, target = edge.target as SimNode; if (source.id === selectedNodeId) result.add(target.id); if (target.id === selectedNodeId) result.add(source.id); });
    return result;
  }, [selectedNodeId, simEdges]);

  return <div ref={container} className="atlas-canvas">
    <TransformWrapper ref={api} minScale={.18} maxScale={3.4} initialScale={.7} limitToBounds={false} centerZoomedOut={false} doubleClick={{ disabled: true }} wheel={{ step: .1 }} panning={{ velocityDisabled: true }} onTransformed={(_, state) => setScale(state.scale)}>
        ...
        <svg width={size.width} height={size.height} role="img" aria-label="Interactieve workspace graph">
          ...
          <g>{simNodes.map((node) => { if (hidden(node) || node.x == null || node.y == null) return null; const r = radius(node), selected = node.id === selectedNodeId, isHovered = node.id === hovered, dim = connected && !connected.has(node.id), showLabel = selected || isHovered || scale > .54 || r >= 12; return <g key={node.id} transform={`translate(${node.x},${node.y})`} role="button" tabIndex={0} aria-label={`${node.label}, ${node.sourceType}`} className="atlas-node" opacity={dim ? .12 : 1}
            ...
            onClick={(event) => { event.stopPropagation(); if (dragged.current) { dragged.current = false; return; } onSelectNode(node.id); }} onMouseEnter={() => setHovered(node.id)} onMouseLeave={() => setHovered(null)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onSelectNode(node.id); }}>
              ...
              <circle r={Math.max(2.2, r * .2)} fill={nodeColorVar(node)} />
    ...
    <div className="atlas-zoom"><button onClick={() => api.current?.zoomIn(.28)} aria-label="Inzoomen"><Plus /></button><button onClick={() => api.current?.zoomOut(.28)} aria-label="Uitzoomen"><Minus /></button><button onClick={() => fit()} aria-label="Passend maken"><LocateFixed /></button></div>
  </div>;
}
