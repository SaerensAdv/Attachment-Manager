...
router.post("/graph/sync", async (req, res): Promise<void> => {
  ...
  try {
    ...
    if (collected.input.docGraph?.nodes) {
    ...
    const excluded = exclusionCount(collected.report); const notes: string[] = []; if (excluded > 0) notes.push(`Begrensd: ${graph.nodes.length} nodes opgenomen, ${excluded} bronitems uitgesloten.`); if (collected.errors.length > 0) notes.push(`Gedeeltelijk: ${collected.errors.length} bron(nen) niet bereikbaar.`); notes.push(`Build ${runtime.gitSha?.slice(0, 8) ?? "unknown"}; lenses S${candidate.nodesByLens.structure} K${candidate.nodesByLens.knowledge} A${candidate.nodesByLens.agents} W${candidate.nodesByLens.active} F${candidate.nodesByLens.flows}.`); res.json(SyncGraphResponse.parse({ ok: true, changed, meta: metaFrom(meta), note: notes.join(" ") }));
