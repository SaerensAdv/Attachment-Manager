...
export interface QueueSummary { pending?: number; queued?: number; processing?: number; retrying: number; failed?: number; deadLetters?: number; succeeded?: number; ignored?: number; [key: string]: unknown }
export interface AtlasOperationsStatus {
  pendingApprovals: number; pendingProposals: number; unresolvedAlerts: number; pushQueue: QueueSummary;
  scheduler: { status: "healthy" | "degraded" | "unknown"; heartbeatAt: string | null; enabledSchedules: number; nextRunAt: string | null; [key: string]: unknown };
...
export interface ClickUpPushRecord { id: number; kind: string; sourceRunId: string | null; status: string; attempts: number; errorCode: string | null; objectId: string | null; objectUrl: string | null; createdAt: string; updatedAt: string; nextAttemptAt: string | null; terminalAt: string | null; [key: string]: unknown }
