...
export function boundTasksByList(input: Array<{ listId: string; tasks: CuTask[] }>, policy: GraphCollectionPolicy, now = new Date()): { tasksByList: Array<{ listId: string; tasks: CuTask[] }>; counts: GraphCollectionReport["tasks"] } {
  const cutoff = now.getTime() - policy.taskLookbackDays * 86_400_000; let discovered = 0, excludedByAge = 0, excludedByListCap = 0; const candidates: Array<{ listId: string; task: CuTask }> = [];
  for (const group of input) { discovered += group.tasks.length; const recent = group.tasks.filter((task) => { const keep = taskTime(task) >= cutoff; if (!keep) excludedByAge += 1; return keep; }).sort((a, b) => taskTime(b) - taskTime(a) || a.id.localeCompare(b.id)); excludedByListCap += Math.max(0, recent.length - policy.maxTasksPerList); for (const task of recent.slice(0, policy.maxTasksPerList)) candidates.push({ listId: group.listId, task }); }
  candidates.sort((a, b) => taskTime(b.task) - taskTime(a.task) || a.task.id.localeCompare(b.task.id)); const selected = candidates.slice(0, policy.maxTasksTotal); const byList = new Map<string, CuTask[]>(); for (const { listId, task } of selected) byList.set(listId, [...(byList.get(listId) ?? []), task]);
  return { tasksByList: [...byList].map(([listId, tasks]) => ({ listId, tasks })), counts: { discovered, included: selected.length, excludedByAge, excludedByListCap, excludedByGlobalCap: Math.max(0, candidates.length - selected.length) } };
}
export function selectDocs(docs: CuDoc[], policy: GraphCollectionPolicy): { docs: CuDoc[]; excluded: number } { const allowed = docs.filter((doc) => allowsDoc(doc.id, policy)); allowed.sort((a, b) => Number(requiresCompletePageTree(b.id, policy)) - Number(requiresCompletePageTree(a.id, policy)) || (Date.parse(b.updatedAt ?? "") || 0) - (Date.parse(a.updatedAt ?? "") || 0) || a.id.localeCompare(b.id)); const selected = allowed.slice(0, policy.maxDocs); return { docs: selected, excluded: docs.length - selected.length }; }
export function boundPageTree(pages: CuDocPage[], max: number): { pages: CuDocPage[]; discovered: number; included: number; excluded: number } {
  let discovered = 0, included = 0; const countOnly = (items: CuDocPage[]): void => { for (const page of items) { discovered += 1; countOnly(page.children); } };
  const walk = (items: CuDocPage[]): CuDocPage[] => { const out: CuDocPage[] = []; for (const page of items) { discovered += 1; if (included >= max) { countOnly(page.children); continue; } included += 1; out.push({ ...page, children: walk(page.children) }); } return out; };
  const bounded = walk(pages); return { pages: bounded, discovered, included, excluded: discovered - included };
}
export function emptyCollectionReport(): GraphCollectionReport { return { workspaces: { discovered: 0, included: 0, excluded: 0 }, spaces: { discovered: 0, included: 0, excluded: 0 }, lists: { discovered: 0, included: 0, excluded: 0 }, tasks: { discovered: 0, included: 0, excludedByAge: 0, excludedByListCap: 0, excludedByGlobalCap: 0 }, docs: { discovered: 0, included: 0, excluded: 0 }, pages: { discovered: 0, included: 0, excluded: 0 }, clients: { included: 0 }, pushRecords: { discovered: 0, included: 0, excluded: 0 } }; }
