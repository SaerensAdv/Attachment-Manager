...
const CrawlTool=()=> <ClientToolRoute tool="crawl"/>; const SearchTermsTool=()=> <ClientToolRoute tool="search-terms"/>;
function Router(){return <Switch><Route path="/" component={WorkspaceGraph}/><Route path="/graph"><Redirect to="/"/></Route><Route path="/atlas"><Redirect to="/"/></Route><Route path="/legacy" component={Home}/><Route path="/generate"><Redirect to="/"/></Route><Route path="/dashboard" component={SystemHealth}/><Route path="/health"><Redirect to="/dashboard"/></Route><Route path="/team" component={Team}/><Route path="/clients/crawl" component={CrawlTool}/><Route path="/clients/search-terms" component={SearchTermsTool}/><Route path="/clients" component={AtlasClients}/><Route path="/crawl"><Redirect to="/clients/crawl"/></Route><Route path="/zoektermen"><Redirect to="/clients/search-terms"/></Route><Route path="/history" component={Runs}/><Route path="/runs"><Redirect to="/history"/></Route><Route path="/todo" component={Operations}/><Route path="/operations"><Redirect to="/todo"/></Route><Route path="/planning" component={Planning}/><Route path="/controle" component={Knowledge}/><Route path="/knowledge"><Redirect to="/controle"/></Route><Route path="/visuals" component={VisualStudio}/><Route component={NotFound}/></Switch>}
const atlasRoutes=new Set(["/","/graph","/atlas","/generate","/todo","/operations","/history","/runs","/clients","/clients/crawl","/clients/search-terms","/crawl","/zoektermen","/dashboard","/health","/controle","/knowledge"]);
...
function App(){return <QueryClientProvider client={queryClient}><TooltipProvider><AuthGate><WouterRouter base={import.meta.env.BASE_URL.replace(/\/$/,"")}><AtlasGenerationProvider><SmoothScroll><AppChrome/><AnimatedRoutes/><AtlasCommandDock/></SmoothScroll></AtlasGenerationProvider></WouterRouter></AuthGate><Toaster/></TooltipProvider></QueryClientProvider>}
export default App;
