...
async function packageRuntimeDocs(distDir) {
  ...
  for (const folder of ["agents", "clients", "workflows", "templates", "knowledge"]) {
    await cp(path.join(workspaceDir, folder), path.join(distDir, folder), { recursive: true, force: true });
  }
}
