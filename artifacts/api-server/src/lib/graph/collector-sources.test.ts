...
describe("Atlas collector source coverage", () => {
  it("turns persisted generations into Active run nodes", () => {
    const graph = buildGraph(base({ runs: [{ id: "42", label: "Monthly reporting", status: "completed", updatedAt: "2026-07-18T18:00:00.000Z" }] }));
    expect(graph.nodes).toContainEqual(expect.objectContaining({ id: "replit:run:42", sourceType: "run", label: "Monthly reporting", status: "completed" }));
  });
