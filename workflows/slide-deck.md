...
Turn an approved narrative into a paste-ready Replit build prompt for a **slide deck** — a professional presentation the Replit Agent builds as a React deck that exports to PPTX, Google Slides, or PDF. A human reviews, exports, and presents; nothing goes live automatically. This workflow's deliverable is a Replit prompt; write it per `knowledge/replit-builds.md` and `knowledge/replit-builds.md`. A pitch/sales/results deck is agency-authored, so apply the Saerens house style from `knowledge/saerens-brand.md`.
...
4. Set the visual direction per slide (chart types and the data behind them, icons, imagery) and the deck theme grounded in the Saerens house style (`knowledge/saerens-brand.md`), the fixed deck layout standard (`knowledge/saerens-brand.md`), and `knowledge/agency-foundations.md`.
