...
- For every mis-routed term, name **both sides**: the cross-campaign negative AND the **positive add** — the term, the correct campaign and **ad group** (read the live ad-group structure to pick a real one), and the suggested match type. Axel adds these by hand, so name them in the analysis; never put positives in the negatives CSV.
- Manage the **persistent monitor list**. Relevant-but-not-converting terms are never excluded outright: escalate by fixing the **landing page** or **bid** first, and only mark a term for exclusion once that escalation is exhausted. Re-emit the monitor list each run in the machine-readable `monitor-list` block (see `workflows/account-optimization.md`) so terms persist with their age.
