...
export const clickupWebhookEventsTable = pgTable(
  "clickup_webhook_events",
  {
    ...
    webhookId: text("webhook_id"), historyId: text("history_id"),
    eventType: text("event_type").notNull(), taskId: text("task_id").notNull(),
    workspaceId: text("workspace_id"), actorId: text("actor_id"),
    eventAt: timestamp("event_at", { withTimezone: true }),
    payload: jsonb("payload").notNull(), payloadHash: text("payload_hash").notNull(),
    status: text("status").notNull().default("queued"), attempts: integer("attempts").notNull().default(0),
    nextAttemptAt: timestamp("next_attempt_at", { withTimezone: true }).defaultNow().notNull(),
    lastError: text("last_error"), receivedAt: timestamp("received_at", { withTimezone: true }).defaultNow().notNull(),
