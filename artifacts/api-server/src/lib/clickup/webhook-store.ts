...
export function ensureClickUpWebhookTable(): Promise<void> {
  ready ??= pool.query(`
    ...
    UPDATE clickup_webhook_events SET status='retrying', next_attempt_at=now(), updated_at=now(), last_error='STALE_PROCESSING_RECOVERED'
...
export interface ClaimedWebhookEvent {
  id: number; idempotencyKey: string; webhookId: string | null; historyId: string | null;
  eventType: string; taskId: string; workspaceId: string | null; actorId: string | null;
  eventAt: Date | null; attempts: number; payload: Record<string, unknown>; payloadHash: string;
}
...
export async function claimNextClickUpWebhookEvent(): Promise<ClaimedWebhookEvent | null> {
  ...
  const result = await pool.query(`
    ...
    WHERE id = (SELECT id FROM clickup_webhook_events WHERE status IN ('queued','retrying') AND next_attempt_at <= now()
      ORDER BY received_at ASC FOR UPDATE SKIP LOCKED LIMIT 1)
    RETURNING id, idempotency_key, webhook_id, history_id, event_type, task_id, workspace_id, actor_id, event_at, attempts, payload, payload_hash
  `);
  const row = result.rows[0];
  return row ? { id: Number(row.id), idempotencyKey: row.idempotency_key, webhookId: row.webhook_id, historyId: row.history_id,
    eventType: row.event_type, taskId: row.task_id, workspaceId: row.workspace_id, actorId: row.actor_id,
    eventAt: row.event_at ? new Date(row.event_at) : null, attempts: row.attempts, payload: row.payload, payloadHash: row.payload_hash } : null;
}
...
export async function failClickUpWebhookEvent(id: number, attempts: number, message: string, retryable: boolean): Promise<"retrying" | "dead_letter"> {
  ...
  await pool.query(`UPDATE clickup_webhook_events SET status=$2, last_error=$3,
...
export async function clickUpWebhookSummary(): Promise<{ queued: number; processing: number; retrying: number; deadLetters: number; succeeded: number; ignored: number; lastEventAt: string | null }> {
  await ensureClickUpWebhookTable();
  const result = await pool.query(`SELECT count(*) FILTER (WHERE status='queued')::int queued,
    count(*) FILTER (WHERE status='processing')::int processing, count(*) FILTER (WHERE status='retrying')::int retrying,
    count(*) FILTER (WHERE status='dead_letter')::int dead_letters, count(*) FILTER (WHERE status='succeeded')::int succeeded,
    count(*) FILTER (WHERE status='ignored')::int ignored, max(received_at) last_event_at FROM clickup_webhook_events`);
  const row = result.rows[0];
  return { queued: row.queued, processing: row.processing, retrying: row.retrying, deadLetters: row.dead_letters,
    succeeded: row.succeeded, ignored: row.ignored, lastEventAt: row.last_event_at ? new Date(row.last_event_at).toISOString() : null };
}
