...
8. **Deeper analytical pass.** Go beyond the surface the way a real analyst would. The live read provides device, day-of-week, hour-of-day and geo (region) splits — read them for patterns (e.g. a device or daypart with a structurally higher CPA, a region that wastes spend). Read trends over time, not just this week's snapshot; question whether a "good" number is real or a vanity metric. The weekly value is in this richer read, not a list of obvious tweaks.
