...
router.post("/clients/clickup/apply",async(req,res)=>{
 const links=Array.isArray(req.body?.links)?req.body.links:[],linked:{clientId:number;companyId:string}[]=[],errors:string[]=[];
 const rows=links.length?await db.select({id:clientsTable.id,companyId:clientsTable.clickupCompanyId}).from(clientsTable):[];
 const taken=new Set(rows.map(r=>(r.companyId??"").trim()).filter(Boolean));
 for(const raw of links){const l=(raw??{}) as Record<string,unknown>,clientId=Number(l.clientId),companyRaw=asTrimmed(l.companyId);if(!Number.isInteger(clientId)||clientId<=0||!companyRaw){errors.push("Ongeldige koppeling overgeslagen.");continue}const checked=validateCompanyId(companyRaw);if(typeof checked!=="string"){errors.push(checked.error);continue}if(taken.has(checked)){errors.push(`ClickUp-bedrijf ${checked} is al aan een klant gekoppeld, overgeslagen.`);continue}
  let updated;try{[updated]=await db.update(clientsTable).set({clickupCompanyId:checked,updatedAt:new Date()}).where(and(eq(clientsTable.id,clientId),or(isNull(clientsTable.clickupCompanyId),eq(clientsTable.clickupCompanyId,"")))).returning()}catch(err){if(isUniqueViolation(err)){taken.add(checked);errors.push(`ClickUp-bedrijf ${checked} is al aan een klant gekoppeld, overgeslagen.`);continue}throw err}
  if(updated){linked.push({clientId,companyId:checked});taken.add(checked);continue}
  const [current]=await db.select().from(clientsTable).where(eq(clientsTable.id,clientId));
  if(!current)errors.push(`Klant ${clientId} niet gevonden.`);else errors.push(`${current.name}: is al aan een ClickUp-bedrijf gekoppeld, overgeslagen.`);
 }
