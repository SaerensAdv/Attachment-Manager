...
const llmLimiter = rateLimit({ windowMs: 60_000, limit: 30, standardHeaders: true, legacyHeaders: false,
  validate: { xForwardedForHeader: false }, message: { error: "Te veel aanvragen na elkaar. Wacht even en probeer opnieuw." } });
...
// ClickUp signs the exact bytes it sends. Mount this before every JSON/text parser
// and before session auth; HMAC + policy authorization are its authentication.
