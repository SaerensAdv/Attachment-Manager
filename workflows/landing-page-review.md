# Workflow: Landing Page Review

## Goal

Review a landing page against its traffic source and goal, and produce prioritized, conversion-focused recommendations so the clicks Saerens drives (paid and organic) convert better.

## When to use

Before launching a campaign that points to a key page, when a campaign's traffic converts poorly, during an audit, or when a client plans a new or redesigned landing page.

## Steps

1. Identify the client, the page(s) under review, and the traffic source/intent (`clients/<client>.md`).
2. Confirm the page's primary goal and conversion action.
3. Gather current conversion data if available (conversion rate, bounce, form completion, device split).
4. Check message match between the traffic source intent and the page.
5. Review conversion essentials per `knowledge/landing-page-standards.md` (value prop, above-the-fold, CTA, trust, form/checkout friction).
6. Recommend page structure and section order for the goal.
7. Note copy and layout changes (hand copy to the Copywriter).
8. Review technical/UX basics (mobile, speed, accessibility).
9. Propose A/B test hypotheses, labeled as hypotheses.
10. Prioritize recommendations by impact and effort.

## Agents involved

- Orchestrator Agent (routes and briefs)
- Landing Page / Web Design Specialist (lead)
- Copywriter (page copy)
- SEO Specialist (organic/technical overlap, where relevant)

## Required output

Follow the output structure in `knowledge/landing-page-standards.md`. Must include:

- Summary of how well the page serves its goal
- Message match assessment
- Conversion review (value prop, above-the-fold, CTA, trust, friction)
- Structure recommendation
- Copy & layout notes (copy handed to Copywriter)
- Technical / UX findings
- Prioritized recommendations (impact / effort)
- Test hypotheses (labeled)
- Open questions / missing data
- Human approval required (live-site or tracking changes)
