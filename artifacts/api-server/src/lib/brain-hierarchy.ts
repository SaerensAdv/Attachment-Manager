...
export function validateBrainHierarchy(manifest: BrainHierarchyManifest, sources: readonly string[]): BrainHierarchyResult {
  ...
  for (const node of manifest.nodes) {
    if (byId.has(node.id)) pushUnique(issues, { code: "duplicate_id", message: `Duplicate hierarchy id: ${node.id}`, nodeId: node.id }); else byId.set(node.id, node);
    for (const alias of node.aliases ?? []) {
      if (alias === node.id || allIds.has(alias) || aliasOwner.has(alias)) pushUnique(issues, { code: "alias_collision", message: `Hierarchy alias collides: ${alias}`, nodeId: node.id });
  ...
  else { if (root.kind !== "master") pushUnique(issues, { code: "invalid_root_kind", message: "Root node must be kind master", nodeId: root.id }); if (root.parent !== null) pushUnique(issues, { code: "root_has_parent", message: "Root node cannot have a parent", nodeId: root.id }); }
  ...
  for (const node of manifest.nodes) {
    const seen = new Set<string>(); let cursor: BrainHierarchyNode | undefined = node;
    while (cursor) { if (seen.has(cursor.id)) { pushUnique(issues, { code: "cycle", message: `Hierarchy cycle at ${cursor.id}`, nodeId: node.id }); break; } seen.add(cursor.id); if (cursor.parent === null) { if (cursor.id !== manifest.rootId) pushUnique(issues, { code: "detached_tree", message: `Node does not reach root ${manifest.rootId}`, nodeId: node.id }); break; } cursor = byId.get(cursor.parent); if (!cursor) break; }
  }
