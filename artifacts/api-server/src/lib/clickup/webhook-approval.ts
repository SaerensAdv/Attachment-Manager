...
function generationIdFromTask(task: { custom_fields?: { name?: string; value?: unknown }[] }, fieldName: string): number | null {
  ...
  const value = field?.value; const raw = typeof value === "number" || typeof value === "string" ? String(value) : "";
  const id = Number(raw); return Number.isInteger(id) && id > 0 ? id : null;
}
export async function resolveAuthorizedGeneration(taskId: string, policy: ClickUpWebhookPolicy, correlationId: string): Promise<number> {
  ...
  const liveStatus = (result.data.status?.status ?? "").trim().toLowerCase();
  if (liveStatus !== policy.approvalStatus) throw new WebhookProcessingError("LIVE_STATUS_NO_LONGER_APPROVED", false);
...
export async function createApprovedGmailDraft(generationId: number): Promise<void> {
  ...
  let raw: unknown; try { raw = JSON.parse(row.pendingDelivery); } catch { raw = null; }
  ...
  if (!report && !reply && !seoReport) { await revertGenerationApprovalToPending(generationId); throw new WebhookProcessingError("PENDING_DELIVERY_UNREADABLE", false); }
  try { if (reply) await draftEmailReply(reply); else if (seoReport) await draftSeoReport(seoReport); else if (report) await draftMonthlyReport(report); }
  catch (error) { await revertGenerationApprovalToPending(generationId).catch(() => undefined); throw new WebhookProcessingError(`GMAIL_DRAFT_FAILED:${error instanceof Error ? error.message : String(error)}`, true); }
  await setGenerationApproval(generationId, { status: "approved", clearPending: true });
  await appendGenerationStep(generationId, { agentPath: row.workflowPath, agentTitle: "ClickUp-goedkeuring, concept klaargezet in Gmail",
    role: "deliverable", status: "completed", durationMs: null, inputTokens: null, outputTokens: null, charCount: null, errorMessage: null, handoffBrief: null });
}
