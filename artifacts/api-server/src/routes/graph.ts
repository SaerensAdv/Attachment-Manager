...
router.post("/graph/sync", async (req, res): Promise<void> => {
  ...
  try {
    ...
    if (!collected.ok) { await failSync(buildingId, collected.errors.join("; ").slice(0, 300) || "source crawl failed"); res.status(502).json(SyncGraphResponse.parse({ ok: false, changed: false, meta: metaFrom(getActiveGraph()?.meta), note: "Bronnen niet bereikbaar; vorige graph behouden." })); return; }
    const baseGraph = buildGraph(collected.input);
    const hierarchySources = collected.input.docGraph?.nodes?.map((node) => node.path);
    let graph = baseGraph;
    if (hierarchySources) {
      const hierarchy = loadBrainHierarchy(hierarchySources);
    ...
    const runtime = getRuntimeProvenance(); const candidate = diagnoseGraph(graph, runtime);
    ...
    const active = getActiveGraph(); const activeDiagnostics = diagnoseGraph(active?.graph ?? EMPTY_GRAPH, runtime);
