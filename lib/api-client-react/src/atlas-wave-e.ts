...
export interface AtlasSystemStatus {
  status: Exclude<HealthState, "unknown">;
...
export interface AtlasOperationsStatus {
  ...
  pushQueue: QueueSummary;
  scheduler: { heartbeatAt: string | null; healthy: boolean; enabledSchedules: number; nextRunAt: string | null; [key: string]: unknown };
  graph: { lastSyncedAt: string | null; syncing: boolean };
  webhook: { status: string; registered: boolean; configured?: boolean; lastEventAt: string | null; deadLetters: number; [key: string]: unknown };
}
export type CompanySyncState = "never" | "running" | "succeeded" | "partial" | "failed";
export interface CompanySyncStatus {
  status: CompanySyncState;
...
export interface ClickUpCompanyMirror {
...
export interface ClickUpPushRecord {
  id: number; kind: string; sourceRunId: string | null; status: string; attempts: number;
  errorCode: string | null; objectId: string | null; objectUrl: string | null;
  createdAt: string; updatedAt: string; nextAttemptAt: string | null; terminalAt: string | null;
  [key: string]: unknown;
}
export interface ClickUpPushList { records: ClickUpPushRecord[] }

/** Human-facing contract: approval creates a Gmail draft. It never claims delivery. */
export const ATLAS_APPROVAL_COPY = {
  action: "Approve and create Gmail draft",
  success: "Gmail draft created",
...
export const getAtlasClickUpPushes = (query: { status?: string; kind?: string; sourceRunId?: string; limit?: number } = {}, signal?: AbortSignal) => {
  const params = new URLSearchParams();
  Object.entries(query).forEach(([key, value]) => { if (value !== undefined) params.set(key, String(value)); });
  const suffix = params.size ? `?${params.toString()}` : "";
  return customFetch<ClickUpPushList>(`/api/clickup/pushes${suffix}`, { method: "GET", responseType: "json", signal });
};
