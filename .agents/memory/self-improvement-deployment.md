...
**Therefore: deploy these on a Reserved VM** (always-on + persistent writable disk) and set `AUTONOMOUS_TRIGGER_SECRET` in the deployment. As of Jun 2026 `.replit` `deploymentTarget` is still `autoscale`.
