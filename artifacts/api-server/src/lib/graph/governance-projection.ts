...
function governanceMetadata(id: string, kind: string, owner: string, lifecycle: string): Record<string, unknown> { return { governanceId: id, governanceKind: kind, canonicalOwner: owner, lifecycle }; }
...
export function applyGovernanceProjection(graph: Graph, governance: BrainGovernanceResult): Graph {
  ...
  for (const object of governance.manifest.objects) {
    ...
    if (existing) { existing.metadata = { ...existing.metadata, ...metadata }; if (!existing.status) existing.status = object.lifecycle; if (!existing.url && object.graph.url) existing.url = object.graph.url; }
    else { const node: GraphNode = { id: object.graph.id, source: object.graph.source, sourceType: object.graph.sourceType, label: object.label, status: object.lifecycle, ...(object.graph.url ? { url: object.graph.url } : {}), metadata }; nodes.set(node.id, node); }
    objectToGraph.set(object.id, object.graph.id);
  }
  for (const link of governance.manifest.links) {
    const sourceId = objectToGraph.get(link.from); const targetId = objectToGraph.get(link.to);
