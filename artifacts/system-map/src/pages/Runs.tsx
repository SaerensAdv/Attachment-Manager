...
import {
  getGetGenerationQueryKey, getGetGenerationsQueryKey, getGetGenerationStepsQueryKey, getGetProposalsQueryKey,
  useAcceptProposal, useApproveGeneration, useCreateProposals, useDeleteGeneration, useGetGeneration,
  useGetGenerations, useGetGenerationSteps, useGetProposals, useRejectProposal, useRequestGenerationChanges,
  useSetGenerationFeedback, type GenerationStep, type GenerationSummary, type ImprovementProposal,
} from "@workspace/api-client-react";
import ReactMarkdown from "react-markdown";
import remarkGfm from "remark-gfm";
import { AlertTriangle, ArrowLeft, Check, CheckCircle2, ChevronRight, Clipboard, Clock3, Copy, Crown, Download, Eye, FileText, GitPullRequestArrow, Loader2, MailCheck, Package, Radio, RefreshCw, Sparkles, ThumbsDown, ThumbsUp, Trash2, User, X } from "lucide-react";
...
type ApprovalRun = {
  ...
  pendingEmailReply?: { recipient: string; subject: string; inboundText: string; replyBody: string } | null;
};
...
const errorDetail = (error: unknown) => { if (error && typeof error === "object") { const data = (error as { data?: Record<string, unknown> }).data; if (typeof data?.detail === "string") return data.detail; if (typeof data?.error === "string") return data.error; const message = (error as { message?: unknown }).message; if (typeof message === "string") return message; } return "The action failed. Nothing was changed."; };
const roleIcon = (role: string) => role === "lead" ? Crown : role === "deliverable" ? Package : User;

function Approval({ run, refresh }: { run: ApprovalRun; refresh: () => void }) {
  const [note, setNote] = useState(""); const [error, setError] = useState<string | null>(null);
  const approve = useApproveGeneration(); const changes = useRequestGenerationChanges(); const busy = approve.isPending || changes.isPending;
...
function Learning({ runId, verdict: savedVerdict, note: savedNote }: { runId: number; verdict: string | null; note: string | null }) {
  ...
  const [note, setNote] = useState(savedNote ?? ""); const [confirm, setConfirm] = useState<number | null>(null);
...
export default function Runs() {
  ...
  const [tab, setTab] = useState<"output" | "audit" | "learning">("output"); const [confirmDelete, setConfirmDelete] = useState(false); const [copied, setCopied] = useState(false);
  ...
  return <AtlasShell title="Runs" subtitle="Execution history, approvals and learning" actions={actions}><main className="runs-stage">
    <aside className={`runs-index${selected ? " has-selection" : ""}`} data-lenis-prevent><header><div><p>Run archive</p><h2>{runs.length} executions</h2></div></header>
      {list.isLoading && <div className="runs-index-state"><Loader2 className="atlas-rotating" />Loading runs</div>}{list.isError && <div className="runs-index-state is-error"><AlertTriangle />Archive unavailable<button onClick={() => list.refetch()}>Try again</button></div>}{!list.isLoading && !list.isError && !runs.length && <div className="runs-index-state"><Clipboard />No runs archived yet</div>}
    ...
    <section className={`runs-inspector${selected ? " is-open" : ""}`} data-lenis-prevent>
      {selected == null && <div className="runs-no-selection"><Radio /><h2>Select a run</h2><p>Inspect output, every agent handoff, approval state and proposed learning changes.</p></div>}{selected != null && detail.isLoading && <div className="runs-no-selection"><Loader2 className="atlas-rotating" />Loading run</div>}{selected != null && detail.isError && <div className="runs-no-selection is-error"><AlertTriangle /><h2>Run unavailable</h2><button onClick={() => detail.refetch()}>Try again</button></div>}
      {detail.data && <><header className="runs-detail-head"><button type="button" className="runs-back" onClick={close}><ArrowLeft />Back</button><div><p>Run #{detail.data.id} · {formatDate(detail.data.createdAt)}</p><h2>{detail.data.clientName}</h2><span>{detail.data.teamTitles.join(" → ")}</span></div><div className="runs-head-actions"><button type="button" onClick={() => void copy()} disabled={!markdown}>{copied ? <Check /> : <Copy />}</button><button type="button" onClick={download} disabled={!markdown}><Download /></button><button type="button" onClick={close}><X /></button></div></header>
        ...
        <div className="runs-detail-body">
          ...
          {tab === "audit" && <AuditTrail steps={steps.data?.steps ?? []} />}{tab === "learning" && <Learning runId={detail.data.id} verdict={detail.data.feedbackVerdict} note={detail.data.feedbackNote} />}
