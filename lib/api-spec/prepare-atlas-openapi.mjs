...
const REMOVED_LEGACY_PATHS = new Set(["/visuals/plan", "/visuals/background"]);
...
function pathBlocks(lines) {
  ...
  const flush = () => { if (key && !REMOVED_LEGACY_PATHS.has(key)) blocks.set(key, bucket); };
...
console.log(`Prepared Atlas OpenAPI with ${overlayPaths.blocks.size} Wave E path overrides/additions; legacy Visual Studio paths excluded.`);
