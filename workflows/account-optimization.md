...
4. **Search-term mining → negatives.** Review the search terms report and flag terms that are **irrelevant to the client's intent** — Saerens' primary negative trigger (see `knowledge/google-ads-standards.md`). Default to excluding at **campaign level**; note where a shared list or ad-group-level negative would be cleaner.
...
- **Relevance drives negatives**, not just spend — exclude search terms that don't match the client's intent, by default at campaign level.
