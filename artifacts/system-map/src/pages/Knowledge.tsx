...
function formatDate(value: string | null): string {
  ...
  return Number.isNaN(date.getTime()) ? "Unknown" : new Intl.DateTimeFormat("en-BE", { day: "2-digit", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" }).format(date);
}

function KnowledgeReader({ item, onOpen }: { item: AtlasKnowledgeItem; onOpen: (id: string) => void }) {
  const relations = useMemo(() => item.relations.map((relation) => {
    const outgoing = relation.source === item.nodeId;
    return { id: outgoing ? relation.target : relation.source, kind: relation.kind, outgoing };
  }), [item]);
  ...
  return <article className="knowledge-reader">
    <header className="knowledge-reader-head">
      <div className="knowledge-reader-title">
        <p>{CATEGORY_LABEL[item.category] ?? item.category} · {item.source === "github" ? "GitHub canonical" : "Runtime cache"}</p>
        <h2>{item.title}</h2>
        <span>{item.path}</span>
      </div>
      {item.canonicalUrl && <a href={item.canonicalUrl} target="_blank" rel="noopener noreferrer" className="knowledge-source-link">Open source <ArrowUpRight /></a>}
    </header>

    <div className="knowledge-provenance">
      <span><GitBranch />Read-only</span>
      <span><ShieldCheck />Canonical source protected</span>
      <span>Updated {formatDate(item.updatedAt)}</span>
    </div>

    <div className="knowledge-markdown prose prose-invert max-w-none">
      <MarkdownView content={clean} />
    </div>

    {relations.length > 0 && <footer className="knowledge-relations">
      <h3><Link2 />Direct relations</h3>
      <div>{relations.slice(0, 16).map((relation) => <button key={`${relation.kind}-${relation.id}`} type="button" onClick={() => onOpen(relation.id)}><span>{relation.kind.replace("_", " ")}</span>{relation.id}</button>)}</div>
    </footer>}
  </article>;
}
...
export default function Knowledge() {
  ...
  const initialNode = new URLSearchParams(searchString).get("node");
  ...
  const visible = useMemo(() => {
    ...
    return nodes.filter((node) => (category === "all" || node.category === category) && (!term || node.title.toLowerCase().includes(term) || node.path.toLowerCase().includes(term))).sort((a, b) => a.title.localeCompare(b.title));
  }, [nodes, query, category]);

  useEffect(() => {
    if (!selectedId && nodes.length) {
      const preferred = nodes.find((node) => node.path === "AGENTS.md") ?? nodes[0];
      setSelectedId(preferred.id);
    }
  }, [nodes, selectedId]);

  const item = useQuery({
    queryKey: ["atlas-knowledge-item", selectedId],
    queryFn: ({ signal }) => getAtlasKnowledgeItem(selectedId!, signal),
    enabled: Boolean(selectedId),
  });

  const openNode = (id: string) => {
    const target = nodes.find((node) => node.id === id || node.path === id);
    const next = target?.id ?? id;
    setSelectedId(next);
    navigate(`/controle?node=${encodeURIComponent(next)}`, { replace: true });
  };
    ...
    <main className="knowledge-stage">
      <aside className="knowledge-index">
        ...
        <div className="knowledge-list" data-lenis-prevent>
          ...
          {visible.map((node: DocNode) => <button key={node.id} type="button" className={`knowledge-list-row${selectedId === node.id ? " is-active" : ""}`} onClick={() => openNode(node.id)}>
            <i data-category={node.category}><FileCode2 /></i><span><b>{node.title}</b><small>{node.path}</small></span>{node.active === false && <em>Paused</em>}
          </button>)}
        </div>
      </aside>
      ...
      {showValidation && <aside className="knowledge-validation" data-lenis-prevent>
        <header><div><p>Integrity check</p><h2>Validation</h2></div><button type="button" onClick={() => setShowValidation(false)} aria-label="Close validation"><X /></button></header>
